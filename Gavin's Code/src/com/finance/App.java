...
UserBalance userincome = new UserBalance();
double income = userincome.GetInc();
...
//          CALCULATIONS SCENE
        ...
        Button graph = new Button("Graph results");
...
//Printing out currently just in System. Need Nick to make a text field to print out the projected income post savings
        netAfSavBtn.setOnAction(event -> {
            String savedPC = pSavedTF.getText();
            int savedPercent = 0;

            try {
                savedPercent = Integer.parseInt(savedPC);
            } catch (NumberFormatException e) {
                System.out.println("Invalid input: " + e.getMessage());
            }

            double incomeForCalculation = ub1.GetInc();
            System.out.println("Retrieved income for calculation: " + incomeForCalculation);

            newMonthlyEarn = incomeForCalculation - (incomeForCalculation * (savedPercent / 100.0)); // Store the result in the class-level variable
            monthlySavings = incomeForCalculation * (savedPercent / 100.0);
            System.out.println("New monthly earnings after savings: " + newMonthlyEarn);
        });
        ...
        calcFbBtn.setOnAction(event -> {
        	String savedMonthsStr = calcFBMonthsTF.getText();
             savedMonths = 0;
        	 try {
                 savedMonths = Integer.parseInt(savedMonthsStr);
             } catch (NumberFormatException e) {
                 System.out.println("Invalid input: " + e.getMessage());
             }
        	 System.out.println("These are the months wanted to save:" + savedMonths);
        	 double TotalSaved = savedMonths * monthlySavings;
        	 System.out.println(TotalSaved);
        	 System.out.println("This is the total amount saved in %d months " + savedMonths);
        });
        
        //Start display of the graph which shows months on the x axis and shows total savings on the y axis
        graph.setOnAction(event -> {
        	LineChart<Number, Number> lineChart = createSavingsGraph(); // Create graph
            VBox graphLayout = new VBox(10, lineChart, backBtn2); // Add graph and back button in a VBox
        ...
        System.out.println("This is the worth of your savings if invested throughout that entire time:" + savedwithinterest);
        });
        ...
        AnchorPane.setBottomAnchor(graph,350.0); 
        AnchorPane.setLeftAnchor(graph, 400.0);
        ...
        calculationsPane.getChildren().addAll(calcLbl,netMtlyIncLbl,
                ...
                netAfSavBtn,calcFbBtn, backBtn2, graph, InvestedFunds);
 ...
 // Create the graph based on total savings per month
    private LineChart<Number, Number> createSavingsGraph() {
        ...
        for (int month = 1; month <= savedMonths; month++) {
